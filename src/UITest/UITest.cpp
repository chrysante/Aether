...
struct DrawView: public View {
    ...
    void draw(xui::Rect) override {
        ...
        { // Pentagon
            float2 line[] = { { 90.0, 50.0 },
                              { 62.36, 88.04 },
                              { 17.64, 73.51 },
                              { 17.64, 26.49 },
                              { 62.36, 11.96 } };
            ctx->addLine(line, { .fill = Color::Red() },
                         { .width = 10, .closed = true });
            addOffset({ 550, 0 }, line);
            ctx->addPolygon(line, { .fill = Color::Red() },
                            { .isYMonotone = true });
        ...
        { // Quad
            float2 vertices[] = { { 20, 120 },
                                  ...
                                  { 80, 180 },
                                  { 20, 180 } };
            ctx->addPolygon(vertices, { .fill = Color::Red() },
                            { .isYMonotone = true });
        }
        { // More complex Y monotone polygon
            ...
            ctx->addLine(vertices, { .fill = Color::Orange() },
                         { .closed = true, .width = 5 });
